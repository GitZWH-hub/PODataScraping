...
Login_URL = Zhihu_URL + '/signin'
Captcha_URL = Zhihu_URL + '/captcha.gif'
Hot_page_URL = Zhihu_URL + '/hot'
...
# re_column_url = re.compile(r'^http://zhuanlan\.zhihu\.com/([^/]+)/?$') # 旧版本
re_column_url = re.compile(r'^https?://www\.zhihu\.com/column/([^/]+)/?$')
...
re_hot_page_url = re.compile(r'^https?://www\.zhihu\.com/hot')
...
def parser_author_from_tag2(author):
    author_link = author.find('a', class_='UserLink-link')
    if author_link is None:
        return None, '匿名用户', '', ''
    else:
        author_name = author_link.text
        motto = author.find('div', class_='AuthorInfo-detail').text
        author_motto = motto if motto is not None else ''
        author_url = 'https:' + author_link['href']
        return author_url, author_name, author_motto
...
def get_path(path, filename, mode, default_path, default_name):
    if path is None:
        path = os.path.join(
            os.getcwd() + '\\data', remove_invalid_char(default_path))
    if filename is None:
...
def is_valid_lyh(c):
    return str.isdigit(c) or c == 'w' or c == 'k'


def extract_number_from_string(s):
    '''提取字符串中的数字, 仅适用于只有一个数字'''
    s = "".join(list(filter(is_valid_lyh, s)))
    return int(s)


# 使用自己打开的一个浏览器，再用selenium接管这个浏览器这样就可以完成反爬的处理。
def start_driver(port):
    chrome_options = Options()
    chrome_options.add_argument("--headless")
    chrome_options.add_experimental_option("debuggerAddress", f"127.0.0.1:{port}")  # 前面设置的端口号
    driver = webdriver.Chrome(options=chrome_options)
    return driver


COMMON_DRIVER = start_driver(9222)
