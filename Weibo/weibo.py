...
class Weibo(object):
          ...
          if w['card_type'] == 9:
            ...
            if wb:
              ...
              since_date = datetime.strptime(
                self.user_config['since_date'], '%Y-%m-%d %H:%M')
  ...
  # 考虑只保留五个字段：
  # 'created_time'  ,  'title'(微博没有title)  ,  'url'  ,  'text'  ,  'labels'
  def write_json(self, wrote_count):
    """将爬到的信息写入json文件"""
    # data = {}
    # 这里暂时不修改原本的 get_filepath 函数, 得到的 path 是 ./weibo-crawler/weibo/user_id/user_id.json,
    # 我们只需要取其目录名即可, 即 ./weibo-crawler/weibo/user_id
    ...
    org_weibo_info = self.weibo[wrote_count:]
    weibo_info = []
    for weibo in org_weibo_info:
      wb = {
        'create_time': weibo['created_at'],
        'title': str(weibo['id']),  # int 转为 str
        'url': 'https://m.weibo.cn/detail/' + str(weibo['id']),
        'text': weibo['text'],
        'labels': weibo['topics'],
      }
      weibo_info.append(wb)
    for wb in weibo_info:
      cur_path = os.path.join(path, '{}.json'.format(wb['title']))
      with codecs.open(cur_path, 'w', encoding='utf-8') as f:
  ...
  def get_pages(self):
    ...
    try:
      ...
      since_date = datetime.strptime(self.user_config['since_date'],
                                     '%Y-%m-%d %H:%M')
      today = datetime.strptime(datetime.today().strftime('%Y-%m-%d %H:%M'), '%Y-%m-%d %H:%M')
      if since_date <= today:
