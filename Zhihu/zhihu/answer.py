...
class Answer(BaseZhihu):
    ...
    def content(self):
        ...
        content = self.soup.find('div', class_='RichContent-inner').text
        return content
    ...
    def creation_time(self):
        ...
        date = self.soup.find('div', class_='ContentItem-time').a.span.text[4:].strip()
        return date
    ...
    def save(self, filepath=None, filename=None, mode="json"):
        ...
        :rtype: None
        """
        if mode not in ["html", "md", "markdown", 'json']:
            raise ValueError("`mode` must be 'html', 'markdown' or 'md',"
                             " got {0}".format(mode))
        file = get_path(filepath, filename, mode, default_path=self.author.id,
                        default_name=self.question.title + '-' + self.author.name)
        with open(file, 'w') as f:
            if mode == "html":
                raise NotImplementedError
            elif mode == 'json':
                data_dict = {'create_time': self.creation_time, 'title': self.question.title, 'url': self.url,
                             'text': self.content, 'labels': []}
